...
def matriz_escalonada(A):
    ...
    # DOCUMENTAR
    ciclos = numero_filas if (
        numero_filas <= numero_columnas) else numero_columnas
            ...
            for m in range(x + 1, numero_filas):
                if A[m][y] != 0:
                    ...
                    for n in range(y, numero_columnas):
                        A[m][n] = A[m][n] - c * A[x][n]
            # move to next row only  if swap was successful DOCUMENTAR
            x += 1
        y += 1


### Main ###
print("Programa que determina la forma escalonada de una matriz A.\n")
